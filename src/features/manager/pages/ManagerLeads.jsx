...
import { fetchMyExecutives } from '../../../core/services/userService';
...
const ImportLeadsModal = ({ onClose, onImport, isImporting }) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const results = await onImport(selectedFile);
...
const Leads = () => {
  ...
  const { data: executivesPage } = useQuery({
    queryKey: ['executives-list-short'],
    queryFn: () => fetchMyExecutives(0, 100),
  });
  const executives = executivesPage?.content || [];
  ...
  const [bulkAssignExecutiveId, setBulkAssignExecutiveId] = React.useState('');
  ...
  const bulkImportMutation = useMutation({
    mutationFn: (file) => uploadLeadsFile(file),
    onSuccess: () => {
      queryClient.invalidateQueries(['leads']);
    }
  });
    ...
    <div className="space-y-6">
      ...
      {isImportModalOpen && (
        <ImportLeadsModal
          ...
          onImport={(file) => bulkImportMutation.mutateAsync(file)}
          ...
          <div className="bg-white w-full max-w-md rounded-[32px] shadow-2xl p-8 space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 pb-5">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-0.5">Assign {selectedLeads.length} selected leads to an executive.</p>
              </div>
              <button 
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider ml-1">Select Executive</label>
                <div className="relative">
                  <UserPlus className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                  <select
                    value={bulkAssignExecutiveId}
                    onChange={(e) => setBulkAssignExecutiveId(e.target.value)}
                    className="w-full pl-11 pr-4 py-3.5 border border-slate-200 rounded-[20px] focus:outline-none focus:ring-4 focus:ring-primary/5 focus:border-primary/30 transition-all text-sm bg-white cursor-pointer appearance-none"
                  >
            ...
            <div className="pt-4 flex items-center gap-3">
              ...
              <button
                onClick={() => {
                  if (bulkAssignExecutiveId) {
                    bulkAssignMutation.mutate({ leadIds: selectedLeads, targetUserId: bulkAssignExecutiveId });
                  }
                }}
                disabled={!bulkAssignExecutiveId || bulkAssignMutation.isPending}
